...
const bListPkmn = pokémon.filter( p => p.name[0] === "B" ); // a list of pokémon that have names that start with the letter "B"
const pkmnIds = pokémon.map( p => p.id ); // array of just pokémon ids
...
const onlyPoisonType = pokémon.filter( p => p.types.length === 1 && p.types[0] === 'poison' );
